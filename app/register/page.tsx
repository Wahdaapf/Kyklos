...
import { Mail, Lock, User, Users, Eye, EyeOff, Check } from "lucide-react";
  ...
  return (
    <ul className="mt-2 grid grid-cols-1 gap-1 text-xs sm:grid-cols-3">
      {rules.map((r) => (
        <li
          key={r.label}
          className={`flex items-center gap-1.5 ${r.ok ? "text-emerald-600 font-medium" : "text-zinc-400"}`}
        >
...
export default function RegisterPage() {
  ...
  return (
    <AuthShell
      title="Buat komunitas Anda"
      subtitle="Gratis untuk dicoba. Setup kurang dari 2 menit."
      footer={
        <>
          Sudah punya akun?{" "}
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              id="community"
              required
              placeholder="Misal: Arisan Melati, RT 03 Cendana"
              className="h-11 rounded-xl pl-10"
            />
          </div>
        </div>
